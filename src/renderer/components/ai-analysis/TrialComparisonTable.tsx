      ...
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-50">
            <th className="border border-gray-300 px-4 py-2 text-left text-small-medium text-gray-900">
              Assessment Category
            </th>
            {trialComparisons.map((trial, index) => (
              <React.Fragment key={trial.trialNumber}>
                <th className="border border-gray-300 px-4 py-2 text-center text-small-medium text-gray-900">
                  You (Trial {trial.trialNumber})
                </th>
                <th className="border border-gray-300 px-4 py-2 text-center text-small-medium text-gray-900 bg-purple-100">
        ...
        <tbody>
          {currentTemplate.questions.map((question, qIndex) => (
            <tr key={question.id} className={qIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
              <td className="border border-gray-300 px-4 py-2 text-small-medium text-gray-900">
                {question.text}
              </td>
              {trialComparisons.map((trial) => (
                <React.Fragment key={trial.trialNumber}>
                  <td className="border border-gray-300 px-4 py-2 text-center text-small text-gray-900">
                    {trial.humanRatings[question.id] || '-'}
                  </td>
                  <td className="border border-gray-300 px-4 py-2 text-center text-small-medium text-purple-600 bg-purple-50">
