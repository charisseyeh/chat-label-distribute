...
export class IPCHandlers {
  ...
  private setupConversationHandlers() {
    ...
    ipcMain.handle('file:store-json', async (event, filePath: string) => {
      try {
        const storedFile = await this.conversationManager.storeJsonFile(filePath);
    ...
    ipcMain.handle('file:get-stored-files', async () => {
      try {
        const storedFiles = await this.conversationManager.getStoredFiles();
    ...
    ipcMain.handle('file:delete-stored-file', async (event, fileId: string) => {
      try {
        const success = await this.conversationManager.deleteStoredFile(fileId);
    ...
    ipcMain.handle('file:get-storage-stats', async () => {
      try {
        const stats = await this.conversationManager.getStorageStats();
    ...
    ipcMain.handle('file:cleanup-duplicates', async () => {
      try {
        const result = await this.conversationManager.cleanupDuplicateFiles();
    ...
    ipcMain.handle('conversations:store-selected', async (event, selectedConversations: any[]) => {
      try {
        const selectedDir = this.conversationManager.getSelectedConversationsDirectory();
